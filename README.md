...
scenarios/                      runnable example Scenario modules
  example_homogeneous.py
  example_paired_comparison.py
src/sim/                        core simulator
  ...
  policy.py                     Policy ABC + BaselinePolicy
  market.py                     regional demand/supply (composes freshness × stage × season × promo × cross)
  event_engine.py               stochastic disruptions + scheduled deliveries
  item_registry.py              catalog + lifecycle transitions (delegates to LifecycleClock)
...
src/llm/                        LLM world builder
  world_builder.py              four-stage orchestrator
  schemas.py                    Pydantic schemas (Taxonomy, Catalog, StoreTemplateList, MarketDomain)
  prompts.py                    prompt templates with retry support
  openai_client.py              OpenAI structured-output client
tests/                          pytest suite
```
...
1. **`build_taxonomy()`** — one LLM call. Returns a `Taxonomy` with categories and `target_share` weights for the archetype.
2. **`sample_catalog(n)`** — two-step. A deterministic Python allocator (`allocate_skeletons`) distributes `n` slots across taxonomy categories proportional to `target_share`; one LLM call then names and refines those slots into concrete SKUs, and authors per-`Ware` lifecycle / freshness / stock fields (`init_stage`, `stage_change_probs`, `freshness_alpha`, `freshness_decay`, `init_stock_share`) per category. Output is a `list[Ware]` with stable `P{i:04d}` ids assigned by `load_catalog`.
3. **`build_store_templates()`** — one LLM call. Returns `dict[str, StoreTemplate]` keyed by template id (e.g. `"flagship"`, `"standard"`, `"outlet"`), one entry per region. Each template carries an explicit `init_active_products` roster and an `init_freshness` mode (`"baseline"` for established stores, `"fresh"` for grand-opening).
4. **`build_market_domain_params()`** — one LLM call. The LLM authors a domain-meaningful slice (`cycle_len`, `peak_factor`, `off_factor`, `init_demand`, `init_supply`, `season_months`, `regions`, `price_elasticity`); `WorldBuilder` merges that with hand-set math defaults (clamps, divisors, distribution objects, lifecycle stage multipliers including `dead`) defined in `world_builder.py:_MARKET_MATH_DEFAULTS` to produce a complete `MarketParams`.

`build(n_items)` runs them in order **market → templates → taxonomy → catalog** because the market call sets the region list that the templates call consumes. Each stage caches its result on the instance, so calling `build()` twice does not re-hit the LLM.

### Schemas and validation

Every LLM payload is parsed through a Pydantic model in `src/llm/schemas.py`. Validators currently enforced:

- `CatalogItem.base_price > unit_cost` (positive margin) and `unit_cost >= 0`
- `Catalog.related_products` references must resolve to other items in the same catalog (no dangling refs, no self-references)
- `CatalogItem.seasonality` is a closed enum (`spring`, `summer`, `fall`, `winter`, `spring/summer`, `fall/winter`, `all_season`)
- `CatalogItem.init_stage` is a closed enum over `[introduction, growth, maturity, decline, dead]`; `freshness_alpha >= 0`, `init_stock_share >= 0`; `stage_change_probs` keys mirror `lifecycle_clock.CANONICAL_STAGES`
- `RelatedRef.correlation ∈ [0, 1]`
- `TaxonomyCategory.target_share ∈ (0, 1]`
- `StoreTemplateList` template ids are unique; each template's `init_active_products` is a non-empty list and `init_freshness` is `"baseline"` or `"fresh"`
- `MarketDomain.price_elasticity < 0`
- `MarketDomain.season_months` is a list of `{name, months}` records (not a free-form dict — OpenAI strict-output mode rejects open-ended object keys)
...
- Default model: `gpt-4o-2024-08-06`. Override via `OpenAIClient(model="…")`.
