...
4. Build the dependency graph. **Validate blocker IDs first**: every `Blocked by` reference must resolve to an issue file under `issues/` or a `done/` entry — an unresolvable ID would silently exclude its issue from every wave. Unknown IDs → ask the user. **Cycles → ask the user.** Cycles indicate `/to-issues` produced non-vertical slices and need human review; do not invent a tie-breaker.
...
7. **Pre-dispatch consistency scan** (cheap — this is the last attended moment, so catch what the unattended run can't ask about): for every file path an issue mandates committing, run `git check-ignore -q <path>` — a hit means the artifact would be silently untracked; and skim each issue against the PRD for direct contradictions (artifact locations, on-disk homes, scope statements). Fold any findings into the wave-plan question (step 8).
8. **Present the wave plan to the user** — annotate each issue with its resolved implementer model so a complexity-driven `opus` pick is visible — and let them abort, reorder waves, drop issues, adjust the parallelism cap, or override any per-issue model. This is the **last point at which questions are allowed** — once you start Phase 2, the run is unattended.
